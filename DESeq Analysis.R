...
data <- fread("Qianxia_raw_counts_by_Genes_clean.csv")
DESeq.data.1 <- data
DESeq.data.1 <- DESeq.data.1[, -c(2:6)]
DESeq.data.1 <- DESeq.data.1[, c(1:64)]
DESeq.data.1 <- as.data.frame(DESeq.data.1[, -c(10,14,35)]) # A46: 42
DESeq.data.1 <- DESeq.data.1[, -c(9,38,44,45,56)] # A46 at column 44
...
# Write out results
#write.csv(resLFC.1,"Additional Files/DESeq2 results baseline.csv")
...
plot(log2(normal_RC.1$A41+1), log2(normal_RC.1$A20+1), pch=16, cex=0.8)
abline(0,1, col=2)
# A41
...
#write.csv(resLFC.2,"Additional Files/DESeq2 results 3 months.csv")
...
plot(log2(normal_RC.2$B41+1), log2(normal_RC.2$B20+1), pch=16, cex=0.8)
abline(0,1, col=2)
...
DESeq.data.3 <- data
DESeq.data.3 <- DESeq.data.3[, c(1,131:163)]
DESeq.data.3 <- as.data.frame(DESeq.data.3[, -c(3,19,27)]) # remove C09, D13, D34
...
#write.csv(resLFC.3,"Additional Files/DESeq2 results time 3.csv")
...
#lot(log2(normal_RC.3$C41+1), log2(normal_RC.3$C20+1), pch=16, cex=0.8)
#abline(0,1, col=2)
...
# Split into subsets
metaData.sub.1 <- data.frame(Final[1:56,c(2,4)])
metaData.sub.1 <- metaData.sub.1[-41,]  # A46 at row 41
rownames(metaData.sub.1) <- NULL
colnames(metaData.sub.1)[2] <- "Pheno"
metaData.sub.1$pr1 <- FALSE
metaData.sub.1$pr1[c(1,4,12,13,22,23,27,28,36,43)] <- TRUE #assign first subgroup of progressing subjects
metaData.sub.1$pr2 <- FALSE
metaData.sub.1$pr2[c(5,16,17,32,34,38,44,50)] <- TRUE #assign second subgroup
...
DESeq.paired.2.np <- DESeq.paired.2.p
...
sig.genes.1 <- normal_RC.1[resLFC.1$padj<0.1,] %>% drop_na()
setDT(sig.genes.1, keep.rownames=T)
write_csv(sig.genes.1, 'Additional Files/time1_data.csv')


sig.genes.3 <- normal_RC.3[resLFC.3$padj<0.1,] %>% drop_na()
setDT(sig.genes.3, keep.rownames=T)
write_csv(sig.genes.3, 'Additional Files/time3_data.csv')
